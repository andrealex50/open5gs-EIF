...
int eif_sbi_open(void)
{
    ...
    if (ogs_sbi_nf_service_is_available(OGS_SBI_SERVICE_NAME_NEIF_EVENTEXPOSURE)) {
        service = ogs_sbi_nf_service_build_default(
                    nf_instance, OGS_SBI_SERVICE_NAME_NEIF_EVENTEXPOSURE);
        ogs_assert(service);
...
void eif_sbi_close(void)
{
    ogs_sbi_client_stop_all();
    ogs_sbi_server_stop_all();
}

bool eif_sbi_send_request(
...
int eif_sbi_discover_and_send(
        ogs_sbi_service_type_e service_type,
        ogs_sbi_discovery_option_t *discovery_option,
        ogs_sbi_request_t *(*build)(eif_sess_t *sess, void *data),
        eif_sess_t *sess, ogs_sbi_stream_t *stream, void *data)
{
    ...
    if (!xact) {
        ogs_error("eif_sbi_discover_and_send() failed");
        ogs_assert(true ==
            ogs_sbi_server_send_error(stream,
                OGS_SBI_HTTP_STATUS_GATEWAY_TIMEOUT, NULL,
                "Cannot discover", sess->sub_id, NULL));
        return OGS_ERROR;
    }
...
void eif_sbi_send_response(ogs_sbi_stream_t *stream, int status)
{
