...
void eif_state_initial(ogs_fsm_t *s, eif_event_t *e)
{
    eif_sm_debug(e);
...
void eif_state_final(ogs_fsm_t *s, eif_event_t *e)
{
    eif_sm_debug(e);
...
void eif_state_operational(ogs_fsm_t *s, eif_event_t *e)
{
    int rv;
        ...
        CASE(OGS_SBI_SERVICE_NAME_NEIF_EVENTEXPOSURE)
            ...
            CASE(OGS_SBI_RESOURCE_NAME_SUBSCRIPTIONS) 
                if (message.h.resource.component[1]) {
                    /* it has subId: /subscriptions/{subId} */
                    sess = eif_sess_find_by_sub_id(message.h.resource.component[1]);
                    if (!sess) {
                        ogs_error("Not found [%s:%s]", message.h.method, message.h.uri);
                        ogs_assert(true ==
                            ogs_sbi_server_send_error(stream,
                    ...
                    eif_neif_ee_handle_subscriptions(sess, stream, &message);
                } else {
                    /* it doesn't have subId: /subscriptions */
                    SWITCH(message.h.method)
                    CASE(OGS_SBI_HTTP_METHOD_GET)
                        eif_neif_ee_handle_get_subscriptions(stream, &message);
                        break;
                    CASE(OGS_SBI_HTTP_METHOD_POST)
                        if (message.EnergyEeSubsc) {
                            ...
                            ogs_error("No EnergyEeSubsc [%s:%s]", message.h.method, message.h.uri);
                            ogs_assert(true ==
                                ogs_sbi_server_send_error(stream,
                                    OGS_SBI_HTTP_STATUS_BAD_REQUEST,
                                    &message, "No EnergyEeSubsc", message.h.uri, NULL));
                        }
                        break;
                    DEFAULT
                        ogs_error("Invalid HTTP method [%s]", message.h.method);
            ...
            DEFAULT
                ...
                ogs_assert(true ==
                    ogs_sbi_server_send_error(stream,
                        OGS_SBI_HTTP_STATUS_BAD_REQUEST, &message,
                        "Invalid resource name", message.h.resource.component[0], NULL));
            END
            break;
     ...
     */
        ...
        CASE(OGS_SBI_SERVICE_NAME_NNRF_DISC)
            ...
            CASE(OGS_SBI_RESOURCE_NAME_NF_INSTANCES)
                ...
                CASE(OGS_SBI_HTTP_METHOD_GET)
                    if (message.res_status == OGS_SBI_HTTP_STATUS_OK)
                        eif_nnrf_handle_nf_discover(sbi_xact, &message);
                    else
                        ogs_error("HTTP response error [%d]",
                                message.res_status);
                    break;
    ...
    default:
        ogs_error("No handler for event %s", eif_event_get_name(e));
        break;
    }
}
