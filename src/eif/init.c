...
static void eif_main(void *data);
...
#define APP_NAME "eif"
    ...
    ogs_sbi_context_init(OpenAPI_nf_type_EIF);
    eif_context_init();
    ...
    rv = eif_context_parse_config();
    if (rv != OGS_OK) return rv;

    rv = eif_sbi_open();
    if (rv != 0) return OGS_ERROR;

    thread = ogs_thread_create(eif_main, NULL);
...
static void eif_main(void *data)
{
    ...
    ogs_fsm_init(&eif_sm, eif_state_initial, eif_state_final, 0);
    ...
    for ( ;; ) {
        ...
        for ( ;; ) {
            ...
            ogs_assert(e);
            ogs_fsm_dispatch(&eif_sm, e);
            ogs_event_free(e);
        }
    }
done:

    ogs_fsm_fini(&eif_sm, 0);
}
