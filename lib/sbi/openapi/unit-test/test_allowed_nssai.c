...
allowed_nssai_t* instantiate_allowed_nssai(int include_optional) {
  ...
  if (include_optional) {
    allowed_nssai = allowed_nssai_create(
      list_createList(),
      nssf_ns_selection_allowed_nssai__3GPP_ACCESS
    );
  } else {
    allowed_nssai = allowed_nssai_create(
      list_createList(),
...
void test_allowed_nssai(int include_optional) {
    allowed_nssai_t* allowed_nssai_1 = instantiate_allowed_nssai(include_optional);

	cJSON* jsonallowed_nssai_1 = allowed_nssai_convertToJSON(allowed_nssai_1);
	printf("allowed_nssai :\n%s\n", cJSON_Print(jsonallowed_nssai_1));
	allowed_nssai_t* allowed_nssai_2 = allowed_nssai_parseFromJSON(jsonallowed_nssai_1);
	cJSON* jsonallowed_nssai_2 = allowed_nssai_convertToJSON(allowed_nssai_2);
	printf("repeating allowed_nssai:\n%s\n", cJSON_Print(jsonallowed_nssai_2));
}
