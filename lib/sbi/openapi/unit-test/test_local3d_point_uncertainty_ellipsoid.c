...
local3d_point_uncertainty_ellipsoid_t* instantiate_local3d_point_uncertainty_ellipsoid(int include_optional);
...
local3d_point_uncertainty_ellipsoid_t* instantiate_local3d_point_uncertainty_ellipsoid(int include_optional) {
  local3d_point_uncertainty_ellipsoid_t* local3d_point_uncertainty_ellipsoid = NULL;
  if (include_optional) {
    local3d_point_uncertainty_ellipsoid = local3d_point_uncertainty_ellipsoid_create(
      null,
...
void test_local3d_point_uncertainty_ellipsoid(int include_optional) {
    local3d_point_uncertainty_ellipsoid_t* local3d_point_uncertainty_ellipsoid_1 = instantiate_local3d_point_uncertainty_ellipsoid(include_optional);

	cJSON* jsonlocal3d_point_uncertainty_ellipsoid_1 = local3d_point_uncertainty_ellipsoid_convertToJSON(local3d_point_uncertainty_ellipsoid_1);
	printf("local3d_point_uncertainty_ellipsoid :\n%s\n", cJSON_Print(jsonlocal3d_point_uncertainty_ellipsoid_1));
	local3d_point_uncertainty_ellipsoid_t* local3d_point_uncertainty_ellipsoid_2 = local3d_point_uncertainty_ellipsoid_parseFromJSON(jsonlocal3d_point_uncertainty_ellipsoid_1);
	cJSON* jsonlocal3d_point_uncertainty_ellipsoid_2 = local3d_point_uncertainty_ellipsoid_convertToJSON(local3d_point_uncertainty_ellipsoid_2);
	printf("repeating local3d_point_uncertainty_ellipsoid:\n%s\n", cJSON_Print(jsonlocal3d_point_uncertainty_ellipsoid_2));
}
