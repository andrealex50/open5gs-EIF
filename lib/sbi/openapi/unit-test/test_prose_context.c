...
prose_context_t* instantiate_prose_context(int include_optional) {
  ...
  if (include_optional) {
    prose_context = prose_context_create(
      namf_communication_prose_context__AUTHORIZED,
      namf_communication_prose_context__AUTHORIZED,
      namf_communication_prose_context__AUTHORIZED,
      namf_communication_prose_context__AUTHORIZED,
      namf_communication_prose_context__AUTHORIZED,
...
void test_prose_context(int include_optional) {
    prose_context_t* prose_context_1 = instantiate_prose_context(include_optional);

	cJSON* jsonprose_context_1 = prose_context_convertToJSON(prose_context_1);
	printf("prose_context :\n%s\n", cJSON_Print(jsonprose_context_1));
	prose_context_t* prose_context_2 = prose_context_parseFromJSON(jsonprose_context_1);
	cJSON* jsonprose_context_2 = prose_context_convertToJSON(prose_context_2);
	printf("repeating prose_context:\n%s\n", cJSON_Print(jsonprose_context_2));
}
