...
ue_differentiation_info_t* instantiate_ue_differentiation_info(int include_optional);
...
ue_differentiation_info_t* instantiate_ue_differentiation_info(int include_optional) {
  ...
  if (include_optional) {
    ue_differentiation_info = ue_differentiation_info_create(
      ...
      namf_communication_ue_differentiation_info__STATIONARY,
      namf_communication_ue_differentiation_info__SINGLE_TRANS_UL,
       // false, not to have infinite recursion
...
void test_ue_differentiation_info(int include_optional) {
    ue_differentiation_info_t* ue_differentiation_info_1 = instantiate_ue_differentiation_info(include_optional);

	cJSON* jsonue_differentiation_info_1 = ue_differentiation_info_convertToJSON(ue_differentiation_info_1);
	printf("ue_differentiation_info :\n%s\n", cJSON_Print(jsonue_differentiation_info_1));
	ue_differentiation_info_t* ue_differentiation_info_2 = ue_differentiation_info_parseFromJSON(jsonue_differentiation_info_1);
	cJSON* jsonue_differentiation_info_2 = ue_differentiation_info_convertToJSON(ue_differentiation_info_2);
