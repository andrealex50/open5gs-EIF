...
qos_requirement_t* instantiate_qos_requirement(int include_optional) {
  ...
  if (include_optional) {
    qos_requirement = qos_requirement_create(
      ...
      "a",
      "a",
      namf_communication_qos_requirement__NON_GBR,
      1,
      "a"
    );
  } else {
...
void test_qos_requirement(int include_optional) {
    qos_requirement_t* qos_requirement_1 = instantiate_qos_requirement(include_optional);

	cJSON* jsonqos_requirement_1 = qos_requirement_convertToJSON(qos_requirement_1);
	printf("qos_requirement :\n%s\n", cJSON_Print(jsonqos_requirement_1));
	qos_requirement_t* qos_requirement_2 = qos_requirement_parseFromJSON(jsonqos_requirement_1);
	cJSON* jsonqos_requirement_2 = qos_requirement_convertToJSON(qos_requirement_2);
