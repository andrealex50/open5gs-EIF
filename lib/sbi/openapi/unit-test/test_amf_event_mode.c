...
amf_event_mode_t* instantiate_amf_event_mode(int include_optional) {
  ...
  if (include_optional) {
    amf_event_mode = amf_event_mode_create(
      ...
      1,
      list_createList(),
      namf_communication_amf_event_mode__ACTIVATE
    );
  } else {
    amf_event_mode = amf_event_mode_create(
      null,
...
void test_amf_event_mode(int include_optional) {
    amf_event_mode_t* amf_event_mode_1 = instantiate_amf_event_mode(include_optional);

	cJSON* jsonamf_event_mode_1 = amf_event_mode_convertToJSON(amf_event_mode_1);
	printf("amf_event_mode :\n%s\n", cJSON_Print(jsonamf_event_mode_1));
	amf_event_mode_t* amf_event_mode_2 = amf_event_mode_parseFromJSON(jsonamf_event_mode_1);
	cJSON* jsonamf_event_mode_2 = amf_event_mode_convertToJSON(amf_event_mode_2);
