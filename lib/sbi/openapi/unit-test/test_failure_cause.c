...
void test_failure_cause(int include_optional) {
    failure_cause_t* failure_cause_1 = instantiate_failure_cause(include_optional);

	cJSON* jsonfailure_cause_1 = failure_cause_convertToJSON(failure_cause_1);
	printf("failure_cause :\n%s\n", cJSON_Print(jsonfailure_cause_1));
	failure_cause_t* failure_cause_2 = failure_cause_parseFromJSON(jsonfailure_cause_1);
	cJSON* jsonfailure_cause_2 = failure_cause_convertToJSON(failure_cause_2);
	printf("repeating failure_cause:\n%s\n", cJSON_Print(jsonfailure_cause_2));
}
