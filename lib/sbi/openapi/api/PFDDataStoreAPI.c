...
PFDDataStoreAPI_readPFDData(apiClient_t *apiClient, list_t * appId , char * supp_feat )
{
    ...
    // create the path
    long sizeOfPath = strlen("/application-data/pfds")+1;
    char *localVarPath = malloc(sizeOfPath);
    snprintf(localVarPath, sizeOfPath, "/application-data/pfds");
    ...
    if (appId)
    {
        list_addElement(localVarQueryParameters,appId);
    }
    ...
    //    printf("%s\n","Generic Error");
    //}
    cJSON *PFDDataStoreAPIlocalVarJSON = cJSON_Parse(apiClient->dataReceived);
    if(!cJSON_IsArray(PFDDataStoreAPIlocalVarJSON)) {
        return 0;//nonprimitive container
    }
    list_t *elementToReturn = list_createList();
    cJSON *VarJSON;
    cJSON_ArrayForEach(VarJSON, PFDDataStoreAPIlocalVarJSON)
    {
    ...
    cJSON_Delete( PFDDataStoreAPIlocalVarJSON);
