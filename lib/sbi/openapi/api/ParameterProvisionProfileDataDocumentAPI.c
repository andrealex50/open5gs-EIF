...
ParameterProvisionProfileDataDocumentAPI_queryPPData(apiClient_t *apiClient, char * ueId , char * supported_features )
{
    ...
    // create the path
    long sizeOfPath = strlen("/subscription-data/{ueId}/pp-profile-data")+1;
    char *localVarPath = malloc(sizeOfPath);
    snprintf(localVarPath, sizeOfPath, "/subscription-data/{ueId}/pp-profile-data");
    ...
    cJSON *ParameterProvisionProfileDataDocumentAPIlocalVarJSON = cJSON_Parse(apiClient->dataReceived);
    pp_profile_data_t *elementToReturn = pp_profile_data_parseFromJSON(ParameterProvisionProfileDataDocumentAPIlocalVarJSON);
    cJSON_Delete(ParameterProvisionProfileDataDocumentAPIlocalVarJSON);
