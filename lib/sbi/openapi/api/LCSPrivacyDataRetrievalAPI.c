...
//
lcs_privacy_data_t*
LCSPrivacyDataRetrievalAPI_getLcsPrivacyData(apiClient_t *apiClient, char * ueId , char * supported_features , char * If_None_Match , char * If_Modified_Since )
{
    ...
    // create the path
    long sizeOfPath = strlen("/{ueId}/lcs-privacy-data")+1;
    char *localVarPath = malloc(sizeOfPath);
    snprintf(localVarPath, sizeOfPath, "/{ueId}/lcs-privacy-data");
    ...
    cJSON *LCSPrivacyDataRetrievalAPIlocalVarJSON = cJSON_Parse(apiClient->dataReceived);
    lcs_privacy_data_t *elementToReturn = lcs_privacy_data_parseFromJSON(LCSPrivacyDataRetrievalAPIlocalVarJSON);
    cJSON_Delete(LCSPrivacyDataRetrievalAPIlocalVarJSON);
