...
InfluenceDataStoreAPI_readInfluenceData(apiClient_t *apiClient, list_t * influence_Ids , list_t * dnns , list_t * snssais , list_t * internal_Group_Ids , list_t * supis , char * supp_feat )
{
    ...
    // create the path
    long sizeOfPath = strlen("/application-data/influenceData")+1;
    char *localVarPath = malloc(sizeOfPath);
    snprintf(localVarPath, sizeOfPath, "/application-data/influenceData");
    ...
    if (influence_Ids)
    {
        list_addElement(localVarQueryParameters,influence_Ids);
    }
    ...
    //    printf("%s\n","Generic Error");
    //}
    cJSON *InfluenceDataStoreAPIlocalVarJSON = cJSON_Parse(apiClient->dataReceived);
    if(!cJSON_IsArray(InfluenceDataStoreAPIlocalVarJSON)) {
        return 0;//nonprimitive container
    }
    list_t *elementToReturn = list_createList();
    cJSON *VarJSON;
    cJSON_ArrayForEach(VarJSON, InfluenceDataStoreAPIlocalVarJSON)
    {
    ...
    cJSON_Delete( InfluenceDataStoreAPIlocalVarJSON);
