...
PolicyDataSubscriptionsCollectionAPI_createIndividualPolicyDataSubscription(apiClient_t *apiClient, policy_data_subscription_t * policy_data_subscription )
{
    ...
    // create the path
    long sizeOfPath = strlen("/policy-data/subs-to-notify")+1;
    char *localVarPath = malloc(sizeOfPath);
    snprintf(localVarPath, sizeOfPath, "/policy-data/subs-to-notify");
    ...
    cJSON *PolicyDataSubscriptionsCollectionAPIlocalVarJSON = cJSON_Parse(apiClient->dataReceived);
    policy_data_subscription_t *elementToReturn = policy_data_subscription_parseFromJSON(PolicyDataSubscriptionsCollectionAPIlocalVarJSON);
    cJSON_Delete(PolicyDataSubscriptionsCollectionAPIlocalVarJSON);
