...
LCSPrivacySubscriptionDataAPI_queryLcsPrivacyData(apiClient_t *apiClient, char * ueId , list_t * fields , char * supported_features , char * If_None_Match , char * If_Modified_Since )
{
    ...
    // create the path
    long sizeOfPath = strlen("/subscription-data/{ueId}/lcs-privacy-data")+1;
    char *localVarPath = malloc(sizeOfPath);
    snprintf(localVarPath, sizeOfPath, "/subscription-data/{ueId}/lcs-privacy-data");
    ...
    cJSON *LCSPrivacySubscriptionDataAPIlocalVarJSON = cJSON_Parse(apiClient->dataReceived);
    lcs_privacy_data_t *elementToReturn = lcs_privacy_data_parseFromJSON(LCSPrivacySubscriptionDataAPIlocalVarJSON);
    cJSON_Delete(LCSPrivacySubscriptionDataAPIlocalVarJSON);
