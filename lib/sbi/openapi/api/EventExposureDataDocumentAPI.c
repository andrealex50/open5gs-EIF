...
EventExposureDataDocumentAPI_queryEEData(apiClient_t *apiClient, char * ueId , list_t * fields , char * supported_features )
{
    ...
    // create the path
    long sizeOfPath = strlen("/subscription-data/{ueId}/ee-profile-data")+1;
    char *localVarPath = malloc(sizeOfPath);
    snprintf(localVarPath, sizeOfPath, "/subscription-data/{ueId}/ee-profile-data");
    ...
    cJSON *EventExposureDataDocumentAPIlocalVarJSON = cJSON_Parse(apiClient->dataReceived);
    ee_profile_data_t *elementToReturn = ee_profile_data_parseFromJSON(EventExposureDataDocumentAPIlocalVarJSON);
    cJSON_Delete(EventExposureDataDocumentAPIlocalVarJSON);
