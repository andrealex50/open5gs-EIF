...
AccessAndMobilitySubscriptionDataDocumentAPI_queryAmData(apiClient_t *apiClient, char * ueId , char * servingPlmnId , list_t * fields , char * supported_features , char * If_None_Match , char * If_Modified_Since )
{
    ...
    // create the path
    long sizeOfPath = strlen("/subscription-data/{ueId}/{servingPlmnId}/provisioned-data/am-data")+1;
    char *localVarPath = malloc(sizeOfPath);
    snprintf(localVarPath, sizeOfPath, "/subscription-data/{ueId}/{servingPlmnId}/provisioned-data/am-data");
    ...
    cJSON *AccessAndMobilitySubscriptionDataDocumentAPIlocalVarJSON = cJSON_Parse(apiClient->dataReceived);
    access_and_mobility_subscription_data_t *elementToReturn = access_and_mobility_subscription_data_parseFromJSON(AccessAndMobilitySubscriptionDataDocumentAPIlocalVarJSON);
    cJSON_Delete(AccessAndMobilitySubscriptionDataDocumentAPIlocalVarJSON);
