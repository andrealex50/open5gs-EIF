...
ProSeServiceSubscriptionDataAPI_queryPorseData(apiClient_t *apiClient, char * ueId , char * supported_features , char * If_None_Match , char * If_Modified_Since )
{
    ...
    // create the path
    long sizeOfPath = strlen("/subscription-data/{ueId}/prose-data")+1;
    char *localVarPath = malloc(sizeOfPath);
    snprintf(localVarPath, sizeOfPath, "/subscription-data/{ueId}/prose-data");
    ...
    cJSON *ProSeServiceSubscriptionDataAPIlocalVarJSON = cJSON_Parse(apiClient->dataReceived);
    prose_subscription_data_t *elementToReturn = prose_subscription_data_parseFromJSON(ProSeServiceSubscriptionDataAPIlocalVarJSON);
    cJSON_Delete(ProSeServiceSubscriptionDataAPIlocalVarJSON);
