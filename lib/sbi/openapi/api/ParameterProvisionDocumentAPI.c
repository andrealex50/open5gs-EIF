...
ParameterProvisionDocumentAPI_getppData(apiClient_t *apiClient, char * ueId , char * supported_features , char * If_None_Match , char * If_Modified_Since )
{
    ...
    cJSON *ParameterProvisionDocumentAPIlocalVarJSON = cJSON_Parse(apiClient->dataReceived);
    pp_data_t *elementToReturn = pp_data_parseFromJSON(ParameterProvisionDocumentAPIlocalVarJSON);
    cJSON_Delete(ParameterProvisionDocumentAPIlocalVarJSON);
