...
#include <stdio.h>
#include <ctype.h>
#include "MultipleIdentifiersAPI.h"
...
MultipleIdentifiersAPI_getMultipleIdentifiers(apiClient_t *apiClient, list_t * gpsi_list , char * supported_features )
{
    ...
    // create the path
    long sizeOfPath = strlen("/multiple-identifiers")+1;
    char *localVarPath = malloc(sizeOfPath);
    snprintf(localVarPath, sizeOfPath, "/multiple-identifiers");
    ...
    if (gpsi_list)
    {
        list_addElement(localVarQueryParameters,gpsi_list);
    }
    ...
    //    printf("%s\n","Unexpected error");
    //}
    cJSON *MultipleIdentifiersAPIlocalVarJSON = cJSON_Parse(apiClient->dataReceived);
    if(!cJSON_IsArray(MultipleIdentifiersAPIlocalVarJSON)) {
        return 0;//nonprimitive container
    }
    list_t *elementToReturn = list_createList();
    cJSON *VarJSON;
    cJSON_ArrayForEach(VarJSON, MultipleIdentifiersAPIlocalVarJSON)
    {
    ...
    cJSON_Delete( MultipleIdentifiersAPIlocalVarJSON);
