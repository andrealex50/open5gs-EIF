...
SMFSelectionSubscriptionDataDocumentAPI_querySmfSelectData(apiClient_t *apiClient, char * ueId , char * servingPlmnId , list_t * fields , char * supported_features , char * If_None_Match , char * If_Modified_Since )
{
    ...
    // create the path
    long sizeOfPath = strlen("/subscription-data/{ueId}/{servingPlmnId}/provisioned-data/smf-selection-subscription-data")+1;
    char *localVarPath = malloc(sizeOfPath);
    snprintf(localVarPath, sizeOfPath, "/subscription-data/{ueId}/{servingPlmnId}/provisioned-data/smf-selection-subscription-data");
    ...
    cJSON *SMFSelectionSubscriptionDataDocumentAPIlocalVarJSON = cJSON_Parse(apiClient->dataReceived);
    smf_selection_subscription_data_t *elementToReturn = smf_selection_subscription_data_parseFromJSON(SMFSelectionSubscriptionDataDocumentAPIlocalVarJSON);
    cJSON_Delete(SMFSelectionSubscriptionDataDocumentAPIlocalVarJSON);
