...
LCSMobileOriginatedSubscriptionDataAPI_queryLcsMoData(apiClient_t *apiClient, char * ueId , list_t * fields , char * supported_features , char * If_None_Match , char * If_Modified_Since )
{
    ...
    // create the path
    long sizeOfPath = strlen("/subscription-data/{ueId}/lcs-mo-data")+1;
    char *localVarPath = malloc(sizeOfPath);
    snprintf(localVarPath, sizeOfPath, "/subscription-data/{ueId}/lcs-mo-data");
    ...
    cJSON *LCSMobileOriginatedSubscriptionDataAPIlocalVarJSON = cJSON_Parse(apiClient->dataReceived);
    lcs_mo_data_t *elementToReturn = lcs_mo_data_parseFromJSON(LCSMobileOriginatedSubscriptionDataAPIlocalVarJSON);
    cJSON_Delete(LCSMobileOriginatedSubscriptionDataAPIlocalVarJSON);
