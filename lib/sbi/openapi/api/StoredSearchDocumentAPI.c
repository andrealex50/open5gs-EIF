...
StoredSearchDocumentAPI_retrieveStoredSearch(apiClient_t *apiClient, char * searchId , char * Accept_Encoding )
{
    ...
    // create the path
    long sizeOfPath = strlen("/searches/{searchId}")+1;
    char *localVarPath = malloc(sizeOfPath);
    snprintf(localVarPath, sizeOfPath, "/searches/{searchId}");
    ...
    cJSON *StoredSearchDocumentAPIlocalVarJSON = cJSON_Parse(apiClient->dataReceived);
    stored_search_result_t *elementToReturn = stored_search_result_parseFromJSON(StoredSearchDocumentAPIlocalVarJSON);
    cJSON_Delete(StoredSearchDocumentAPIlocalVarJSON);
