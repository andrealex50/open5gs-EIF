...
SliceSelectionSubscriptionDataRetrievalAPI_getNSSAI(apiClient_t *apiClient, char * supi , char * supported_features , plmn_id_t * plmn_id , int disaster_roaming_ind , char * If_None_Match , char * If_Modified_Since )
{
    ...
    // create the path
    long sizeOfPath = strlen("/{supi}/nssai")+1;
    char *localVarPath = malloc(sizeOfPath);
    snprintf(localVarPath, sizeOfPath, "/{supi}/nssai");
    ...
    cJSON *SliceSelectionSubscriptionDataRetrievalAPIlocalVarJSON = cJSON_Parse(apiClient->dataReceived);
    nssai_t *elementToReturn = nssai_parseFromJSON(SliceSelectionSubscriptionDataRetrievalAPIlocalVarJSON);
    cJSON_Delete(SliceSelectionSubscriptionDataRetrievalAPIlocalVarJSON);
