...
V2XSubscriptionDataAPI_queryV2xData(apiClient_t *apiClient, char * ueId , char * supported_features , char * If_None_Match , char * If_Modified_Since )
{
    ...
    // create the path
    long sizeOfPath = strlen("/subscription-data/{ueId}/v2x-data")+1;
    char *localVarPath = malloc(sizeOfPath);
    snprintf(localVarPath, sizeOfPath, "/subscription-data/{ueId}/v2x-data");
    ...
    cJSON *V2XSubscriptionDataAPIlocalVarJSON = cJSON_Parse(apiClient->dataReceived);
    v2x_subscription_data_t *elementToReturn = v2x_subscription_data_parseFromJSON(V2XSubscriptionDataAPIlocalVarJSON);
    cJSON_Delete(V2XSubscriptionDataAPIlocalVarJSON);
