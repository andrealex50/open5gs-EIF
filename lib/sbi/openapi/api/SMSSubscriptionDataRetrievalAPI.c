...
SMSSubscriptionDataRetrievalAPI_getSmsData(apiClient_t *apiClient, char * supi , char * supported_features , plmn_id_t * plmn_id , char * If_None_Match , char * If_Modified_Since )
{
    ...
    // create the path
    long sizeOfPath = strlen("/{supi}/sms-data")+1;
    char *localVarPath = malloc(sizeOfPath);
    snprintf(localVarPath, sizeOfPath, "/{supi}/sms-data");
    ...
    cJSON *SMSSubscriptionDataRetrievalAPIlocalVarJSON = cJSON_Parse(apiClient->dataReceived);
    sms_subscription_data_t *elementToReturn = sms_subscription_data_parseFromJSON(SMSSubscriptionDataRetrievalAPIlocalVarJSON);
    cJSON_Delete(SMSSubscriptionDataRetrievalAPIlocalVarJSON);
