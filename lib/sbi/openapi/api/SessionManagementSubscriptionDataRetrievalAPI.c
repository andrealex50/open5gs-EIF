...
SessionManagementSubscriptionDataRetrievalAPI_getSmData(apiClient_t *apiClient, char * supi , char * supported_features , snssai_t * single_nssai , char * dnn , plmn_id_t * plmn_id , char * If_None_Match , char * If_Modified_Since )
{
    ...
    // create the path
    long sizeOfPath = strlen("/{supi}/sm-data")+1;
    char *localVarPath = malloc(sizeOfPath);
    snprintf(localVarPath, sizeOfPath, "/{supi}/sm-data");
    ...
    cJSON *SessionManagementSubscriptionDataRetrievalAPIlocalVarJSON = cJSON_Parse(apiClient->dataReceived);
    sm_subs_data_t *elementToReturn = sm_subs_data_parseFromJSON(SessionManagementSubscriptionDataRetrievalAPIlocalVarJSON);
    cJSON_Delete(SessionManagementSubscriptionDataRetrievalAPIlocalVarJSON);
