...
SMFSelectionSubscriptionDataRetrievalAPI_getSmfSelData(apiClient_t *apiClient, char * supi , char * supported_features , plmn_id_t * plmn_id , int disaster_roaming_ind , char * If_None_Match , char * If_Modified_Since )
{
    ...
    // create the path
    long sizeOfPath = strlen("/{supi}/smf-select-data")+1;
    char *localVarPath = malloc(sizeOfPath);
    snprintf(localVarPath, sizeOfPath, "/{supi}/smf-select-data");
    ...
    cJSON *SMFSelectionSubscriptionDataRetrievalAPIlocalVarJSON = cJSON_Parse(apiClient->dataReceived);
    smf_selection_subscription_data_t *elementToReturn = smf_selection_subscription_data_parseFromJSON(SMFSelectionSubscriptionDataRetrievalAPIlocalVarJSON);
    cJSON_Delete(SMFSelectionSubscriptionDataRetrievalAPIlocalVarJSON);
