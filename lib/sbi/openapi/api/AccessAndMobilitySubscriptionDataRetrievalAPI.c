...
AccessAndMobilitySubscriptionDataRetrievalAPI_getAmData(apiClient_t *apiClient, char * supi , char * supported_features , plmn_id_nid_t * plmn_id , list_t * adjacent_plmns , int disaster_roaming_ind , char * If_None_Match , char * If_Modified_Since )
{
    ...
    // create the path
    long sizeOfPath = strlen("/{supi}/am-data")+1;
    char *localVarPath = malloc(sizeOfPath);
    snprintf(localVarPath, sizeOfPath, "/{supi}/am-data");
    ...
    if (adjacent_plmns)
    {
        list_addElement(localVarQueryParameters,adjacent_plmns);
    }
    ...
    cJSON *AccessAndMobilitySubscriptionDataRetrievalAPIlocalVarJSON = cJSON_Parse(apiClient->dataReceived);
    access_and_mobility_subscription_data_t *elementToReturn = access_and_mobility_subscription_data_parseFromJSON(AccessAndMobilitySubscriptionDataRetrievalAPIlocalVarJSON);
    cJSON_Delete(AccessAndMobilitySubscriptionDataRetrievalAPIlocalVarJSON);
