...
#include <stdio.h>
#include <ctype.h>
#include "Class5MBSSubscriptionDataRetrievalAPI.h"
...
Class5MBSSubscriptionDataRetrievalAPI_getMbsData(apiClient_t *apiClient, char * supi , char * supported_features , char * If_None_Match , char * If_Modified_Since )
{
    ...
    // create the path
    long sizeOfPath = strlen("/{supi}/5mbs-data")+1;
    char *localVarPath = malloc(sizeOfPath);
    snprintf(localVarPath, sizeOfPath, "/{supi}/5mbs-data");
    ...
    cJSON *Class5MBSSubscriptionDataRetrievalAPIlocalVarJSON = cJSON_Parse(apiClient->dataReceived);
    mbs_subscription_data_t *elementToReturn = mbs_subscription_data_parseFromJSON(Class5MBSSubscriptionDataRetrievalAPIlocalVarJSON);
    cJSON_Delete(Class5MBSSubscriptionDataRetrievalAPIlocalVarJSON);
