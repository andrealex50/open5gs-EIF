...
QueryNIDDAuthorizationDataGPSIOrExternalGroupIdentifierDocumentAPI_getNiddAuData(apiClient_t *apiClient, char * ueId , snssai_t * single_nssai , char * dnn , char * mtc_provider_information , char * af_id , char * If_None_Match , char * If_Modified_Since )
{
    ...
    // create the path
    long sizeOfPath = strlen("/subscription-data/{ueId}/nidd-authorization-data")+1;
    char *localVarPath = malloc(sizeOfPath);
    snprintf(localVarPath, sizeOfPath, "/subscription-data/{ueId}/nidd-authorization-data");
    ...
    cJSON *QueryNIDDAuthorizationDataGPSIOrExternalGroupIdentifierDocumentAPIlocalVarJSON = cJSON_Parse(apiClient->dataReceived);
    authorization_data_t *elementToReturn = authorization_data_parseFromJSON(QueryNIDDAuthorizationDataGPSIOrExternalGroupIdentifierDocumentAPIlocalVarJSON);
    cJSON_Delete(QueryNIDDAuthorizationDataGPSIOrExternalGroupIdentifierDocumentAPIlocalVarJSON);
