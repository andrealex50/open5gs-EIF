...
LCSBroadcastAssistanceDataTypesRetrievalAPI_getLcsBcaData(apiClient_t *apiClient, char * supi , char * supported_features , plmn_id_t * plmn_id , char * If_None_Match , char * If_Modified_Since )
{
    ...
    // create the path
    long sizeOfPath = strlen("/{supi}/lcs-bca-data")+1;
    char *localVarPath = malloc(sizeOfPath);
    snprintf(localVarPath, sizeOfPath, "/{supi}/lcs-bca-data");
    ...
    cJSON *LCSBroadcastAssistanceDataTypesRetrievalAPIlocalVarJSON = cJSON_Parse(apiClient->dataReceived);
    lcs_broadcast_assistance_types_data_t *elementToReturn = lcs_broadcast_assistance_types_data_parseFromJSON(LCSBroadcastAssistanceDataTypesRetrievalAPIlocalVarJSON);
    cJSON_Delete(LCSBroadcastAssistanceDataTypesRetrievalAPIlocalVarJSON);
