...
UEContextInSMSFDataRetrievalAPI_getUeCtxInSmsfData(apiClient_t *apiClient, char * supi , char * supported_features )
{
    ...
    // create the path
    long sizeOfPath = strlen("/{supi}/ue-context-in-smsf-data")+1;
    char *localVarPath = malloc(sizeOfPath);
    snprintf(localVarPath, sizeOfPath, "/{supi}/ue-context-in-smsf-data");
    ...
    cJSON *UEContextInSMSFDataRetrievalAPIlocalVarJSON = cJSON_Parse(apiClient->dataReceived);
    ue_context_in_smsf_data_t *elementToReturn = ue_context_in_smsf_data_parseFromJSON(UEContextInSMSFDataRetrievalAPIlocalVarJSON);
    cJSON_Delete(UEContextInSMSFDataRetrievalAPIlocalVarJSON);
