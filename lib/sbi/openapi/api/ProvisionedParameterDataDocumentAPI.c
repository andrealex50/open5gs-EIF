...
ProvisionedParameterDataDocumentAPI_modifyPpData(apiClient_t *apiClient, char * ueId , list_t * patch_item , char * supported_features )
{
    ...
    // create the path
    long sizeOfPath = strlen("/subscription-data/{ueId}/pp-data")+1;
    char *localVarPath = malloc(sizeOfPath);
    snprintf(localVarPath, sizeOfPath, "/subscription-data/{ueId}/pp-data");
    ...
    cJSON *ProvisionedParameterDataDocumentAPIlocalVarJSON = cJSON_Parse(apiClient->dataReceived);
    patch_result_t *elementToReturn = patch_result_parseFromJSON(ProvisionedParameterDataDocumentAPIlocalVarJSON);
    cJSON_Delete(ProvisionedParameterDataDocumentAPIlocalVarJSON);
