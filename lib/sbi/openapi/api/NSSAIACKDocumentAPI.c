...
NSSAIACKDocumentAPI_queryNssaiAck(apiClient_t *apiClient, char * ueId , char * supported_features )
{
    ...
    // create the path
    long sizeOfPath = strlen("/subscription-data/{ueId}/ue-update-confirmation-data/subscribed-snssais")+1;
    char *localVarPath = malloc(sizeOfPath);
    snprintf(localVarPath, sizeOfPath, "/subscription-data/{ueId}/ue-update-confirmation-data/subscribed-snssais");
    ...
    cJSON *NSSAIACKDocumentAPIlocalVarJSON = cJSON_Parse(apiClient->dataReceived);
    nssai_ack_data_t *elementToReturn = nssai_ack_data_parseFromJSON(NSSAIACKDocumentAPIlocalVarJSON);
    cJSON_Delete(NSSAIACKDocumentAPIlocalVarJSON);
