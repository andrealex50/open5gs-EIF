...
#include <stdio.h>
#include <ctype.h>
#include "RetrievalOfTheIndividualSharedDataAPI.h"
...
RetrievalOfTheIndividualSharedDataAPI_getIndividualSharedData(apiClient_t *apiClient, set_t * sharedDataId , char * supported_features , char * If_None_Match , char * If_Modified_Since )
{
    ...
    // create the path
    long sizeOfPath = strlen("/shared-data/{sharedDataId}")+1;
    char *localVarPath = malloc(sizeOfPath);
    snprintf(localVarPath, sizeOfPath, "/shared-data/{sharedDataId}");


    // Path Params
    long sizeOfPathParams_sharedDataId =  + strlen("{ sharedDataId }");
    ...
    cJSON *RetrievalOfTheIndividualSharedDataAPIlocalVarJSON = cJSON_Parse(apiClient->dataReceived);
    shared_data_t *elementToReturn = shared_data_parseFromJSON(RetrievalOfTheIndividualSharedDataAPIlocalVarJSON);
    cJSON_Delete(RetrievalOfTheIndividualSharedDataAPIlocalVarJSON);
