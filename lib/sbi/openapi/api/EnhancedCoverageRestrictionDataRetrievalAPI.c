...
EnhancedCoverageRestrictionDataRetrievalAPI_getEcrData(apiClient_t *apiClient, char * supi , char * supported_features , char * If_None_Match , char * If_Modified_Since )
{
    ...
    // create the path
    long sizeOfPath = strlen("/{supi}/am-data/ecr-data")+1;
    char *localVarPath = malloc(sizeOfPath);
    snprintf(localVarPath, sizeOfPath, "/{supi}/am-data/ecr-data");
    ...
    cJSON *EnhancedCoverageRestrictionDataRetrievalAPIlocalVarJSON = cJSON_Parse(apiClient->dataReceived);
    enhanced_coverage_restriction_data_t *elementToReturn = enhanced_coverage_restriction_data_parseFromJSON(EnhancedCoverageRestrictionDataRetrievalAPIlocalVarJSON);
    cJSON_Delete(EnhancedCoverageRestrictionDataRetrievalAPIlocalVarJSON);
