...
ProseSubscriptionDataRetrievalAPI_getProseData(apiClient_t *apiClient, char * supi , char * supported_features , char * If_None_Match , char * If_Modified_Since )
{
    ...
    // create the path
    long sizeOfPath = strlen("/{supi}/prose-data")+1;
    char *localVarPath = malloc(sizeOfPath);
    snprintf(localVarPath, sizeOfPath, "/{supi}/prose-data");
    ...
    cJSON *ProseSubscriptionDataRetrievalAPIlocalVarJSON = cJSON_Parse(apiClient->dataReceived);
    prose_subscription_data_t *elementToReturn = prose_subscription_data_parseFromJSON(ProseSubscriptionDataRetrievalAPIlocalVarJSON);
    cJSON_Delete(ProseSubscriptionDataRetrievalAPIlocalVarJSON);
