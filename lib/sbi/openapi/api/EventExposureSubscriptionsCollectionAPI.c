...
EventExposureSubscriptionsCollectionAPI_createEeSubscriptions(apiClient_t *apiClient, char * ueId , ee_subscription_t * ee_subscription )
{
    ...
    // create the path
    long sizeOfPath = strlen("/subscription-data/{ueId}/context-data/ee-subscriptions")+1;
    char *localVarPath = malloc(sizeOfPath);
    snprintf(localVarPath, sizeOfPath, "/subscription-data/{ueId}/context-data/ee-subscriptions");
    ...
    cJSON *EventExposureSubscriptionsCollectionAPIlocalVarJSON = cJSON_Parse(apiClient->dataReceived);
    ee_subscription_t *elementToReturn = ee_subscription_parseFromJSON(EventExposureSubscriptionsCollectionAPIlocalVarJSON);
    cJSON_Delete(EventExposureSubscriptionsCollectionAPIlocalVarJSON);
...
EventExposureSubscriptionsCollectionAPI_queryeesubscriptions(apiClient_t *apiClient, char * ueId , char * supported_features , list_t * event_types , list_t * nf_identifiers )
{
    ...
    if (nf_identifiers)
    {
        list_addElement(localVarQueryParameters,nf_identifiers);
    }
    list_addElement(localVarHeaderType,"application/json"); //produces
    apiClient_invoke(apiClient,
                    localVarPath,
    ...
    //}
    cJSON *EventExposureSubscriptionsCollectionAPIlocalVarJSON = cJSON_Parse(apiClient->dataReceived);
    if(!cJSON_IsArray(EventExposureSubscriptionsCollectionAPIlocalVarJSON)) {
        return 0;//nonprimitive container
    }
    list_t *elementToReturn = list_createList();
    cJSON *VarJSON;
    cJSON_ArrayForEach(VarJSON, EventExposureSubscriptionsCollectionAPIlocalVarJSON)
    {
