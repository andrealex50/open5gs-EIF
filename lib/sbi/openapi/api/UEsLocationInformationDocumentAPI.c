...
UEsLocationInformationDocumentAPI_queryUeLocation(apiClient_t *apiClient, char * ueId , char * supported_features )
{
    ...
    // create the path
    long sizeOfPath = strlen("/subscription-data/{ueId}/context-data/location")+1;
    char *localVarPath = malloc(sizeOfPath);
    snprintf(localVarPath, sizeOfPath, "/subscription-data/{ueId}/context-data/location");
    ...
    cJSON *UEsLocationInformationDocumentAPIlocalVarJSON = cJSON_Parse(apiClient->dataReceived);
    location_info_t *elementToReturn = location_info_parseFromJSON(UEsLocationInformationDocumentAPIlocalVarJSON);
    cJSON_Delete(UEsLocationInformationDocumentAPIlocalVarJSON);
