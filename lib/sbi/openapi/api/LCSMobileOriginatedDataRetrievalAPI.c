...
LCSMobileOriginatedDataRetrievalAPI_getLcsMoData(apiClient_t *apiClient, char * supi , char * supported_features , char * If_None_Match , char * If_Modified_Since )
{
    ...
    // create the path
    long sizeOfPath = strlen("/{supi}/lcs-mo-data")+1;
    char *localVarPath = malloc(sizeOfPath);
    snprintf(localVarPath, sizeOfPath, "/{supi}/lcs-mo-data");
    ...
    cJSON *LCSMobileOriginatedDataRetrievalAPIlocalVarJSON = cJSON_Parse(apiClient->dataReceived);
    lcs_mo_data_t *elementToReturn = lcs_mo_data_parseFromJSON(LCSMobileOriginatedDataRetrievalAPIlocalVarJSON);
    cJSON_Delete(LCSMobileOriginatedDataRetrievalAPIlocalVarJSON);
