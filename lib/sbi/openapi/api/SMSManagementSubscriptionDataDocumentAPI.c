...
SMSManagementSubscriptionDataDocumentAPI_querySmsMngData(apiClient_t *apiClient, char * ueId , char * servingPlmnId , char * supported_features , char * If_None_Match , char * If_Modified_Since )
{
    ...
    // create the path
    long sizeOfPath = strlen("/subscription-data/{ueId}/{servingPlmnId}/provisioned-data/sms-mng-data")+1;
    char *localVarPath = malloc(sizeOfPath);
    snprintf(localVarPath, sizeOfPath, "/subscription-data/{ueId}/{servingPlmnId}/provisioned-data/sms-mng-data");
    ...
    cJSON *SMSManagementSubscriptionDataDocumentAPIlocalVarJSON = cJSON_Parse(apiClient->dataReceived);
    sms_management_subscription_data_t *elementToReturn = sms_management_subscription_data_parseFromJSON(SMSManagementSubscriptionDataDocumentAPIlocalVarJSON);
    cJSON_Delete(SMSManagementSubscriptionDataDocumentAPIlocalVarJSON);
