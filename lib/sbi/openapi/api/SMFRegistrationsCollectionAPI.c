...
SMFRegistrationsCollectionAPI_querySmfRegList(apiClient_t *apiClient, char * ueId , char * supported_features )
{
    ...
    // create the path
    long sizeOfPath = strlen("/subscription-data/{ueId}/context-data/smf-registrations")+1;
    char *localVarPath = malloc(sizeOfPath);
    snprintf(localVarPath, sizeOfPath, "/subscription-data/{ueId}/context-data/smf-registrations");
    ...
    //    printf("%s\n","Generic Error");
    //}
    cJSON *SMFRegistrationsCollectionAPIlocalVarJSON = cJSON_Parse(apiClient->dataReceived);
    if(!cJSON_IsArray(SMFRegistrationsCollectionAPIlocalVarJSON)) {
        return 0;//nonprimitive container
    }
    list_t *elementToReturn = list_createList();
    cJSON *VarJSON;
    cJSON_ArrayForEach(VarJSON, SMFRegistrationsCollectionAPIlocalVarJSON)
    {
    ...
    cJSON_Delete( SMFRegistrationsCollectionAPIlocalVarJSON);
